...
Untrusted inputs include chat messages, language/location/access choices, snapshot IDs, request headers, and model output.
...
- Crowd snapshots are generated by the server adapter; browser-provided readings, labels, and capacities never enter a prompt. Insight requests accept only a bounded snapshot ID and reject stale IDs.
- AI recommendations use a length-bounded structured-output schema and are grounded against deterministic, phase/trend-aware assessments. Unsupported zones/gates are removed; priority, owner, and recheck interval are restored from trusted logic.
- Output tokens, retries, and provider deadlines are capped to control cost and reduce inconsistent responses.
...
- Fan and staff prompts independently receive the current canonical server snapshot and prohibit invented sensor data.
...
- Recommendations require human control-room verification before operational action; the demo UI models acknowledgement/resolution but cannot execute an action.
...
6. Persist signed recommendation IDs, source citations, acknowledgement state, and immutable audit trails (the demo state is intentionally in-memory only).
